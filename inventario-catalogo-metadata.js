...
let lastInfo=null,lastPropagated=0;
function enhanceGeneralModal(){
 ...
 m.querySelector('[data-ok]')?.addEventListener('click',()=>setTimeout(()=>{
   const categoria=sel.value==='__CUSTOM__'?custom.value.trim():sel.value;if(!categoria)return;
   const info=window.__MARC_CATALOG_GENERAL||{};info.categoria=categoria;info.archivo=document.querySelector('#cat4Input')?.files?.[0]?.name||info.archivo||'';window.__MARC_CATALOG_GENERAL=info;
 },0),true);
}
async function propagate(){
 ...
 const fileName=info.archivo||document.querySelector('#cat4Input')?.files?.[0]?.name;if(!fileName)return;
 const c=await sb.from('catalogos_pdf').select('id').eq('user_id',u.id).eq('nombre',fileName).order('created_at',{ascending:false}).limit(1).maybeSingle();if(c.error||!c.data)return;
 const id=c.data.id;await sb.from('catalogos_pdf').update({informacion_general:{...info},proveedor_nombre:info.proveedor,rubro:info.rubro||null}).eq('id',id).eq('user_id',u.id);
 const r=await sb.from('catalogo_items').select('id,datos_extra,categoria,proveedor').eq('catalogo_id',id).eq('user_id',u.id);if(r.error||!r.data?.length)return;
...
function enhanceEditor(){const m=document.querySelector('#catEditorModal'),rows=document.querySelector('#catEditorRows');if(!m||!rows)return;if(!m.querySelector('.marc-inventory-metadata-banner')){const row=rows.querySelector('.cat-editor-row');if(!row)return;const p=row.querySelector('[data-k="proveedor"]')?.value||'',c=row.querySelector('[data-k="categoria"]')?.value||'';const b=document.createElement('div');b.className='marc-inventory-metadata-banner';b.innerHTML=`<b>Catálogo:</b> proveedor <strong>${esc(p||'—')}</strong> · categoría predeterminada <strong>${esc(c||'—')}</strong><br><small>La categoría y el proveedor se pueden corregir por producto antes de importar.</small>`;m.querySelector('.cat-editor-tools')?.after(b)}}
