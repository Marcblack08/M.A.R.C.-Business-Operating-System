...
drop function if exists public.marc_save_quote(uuid,uuid,text,text,boolean,numeric,text,jsonb);
...
begin
  ...
  for v_item in select value from jsonb_array_elements(p_items) loop
    ...
    if v_type='PRODUCTO' then
      ...
      select name,unit,price,cost into v_name,v_unit,v_unit_price,v_cost from public.marc_inventory where id=v_inventory_id and user_id=v_uid and active=true;
  ...
  if p_quote_id is null then
    ...
    update public.marc_quotes set client_id=p_client_id,title=coalesce(nullif(trim(p_title),''),'Cotización'),status=p_status,tax_enabled=p_tax_enabled,tax_rate=p_tax_rate,subtotal=v_sub,tax=v_tax,total=v_total,notes=p_notes,updated_at=now()
  ...
  for v_item in select value from jsonb_array_elements(p_items) loop
    ...
    insert into public.marc_quote_items(quote_id,user_id,inventory_id,item_type,name,description,quantity,unit,unit_price,cost,line_total)
    values(v_quote.id,v_uid,v_inventory_id,v_type,v_name,v_description,v_quantity,v_unit,v_unit_price,v_cost,v_quantity*v_unit_price);
  end loop;
  return v_quote;
end;
$$;
