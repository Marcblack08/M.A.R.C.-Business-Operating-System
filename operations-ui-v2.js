...
function boot(){improveButtonReliability();const observer=new MutationObserver(enhance);[$("#content"),$("#modal")].filter(Boolean).forEach(root=>observer.observe(root,{childList:true,subtree:true}));later(enhance,120);window.addEventListener("resize",()=>{if(window.innerWidth>760)toggleMore(false)})}
