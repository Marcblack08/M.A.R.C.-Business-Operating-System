const CACHE_NAME = 'marc-pwa-v7-document-reader-2';
  ...
  event.respondWith((async()=>{
    try{
      ...
      if((response.headers.get('content-type')||'').includes('text/html') && new URL(request.url).pathname.endsWith('/index.html')){
        ...
        const injected=html.includes('catalogos-v7.js')?html:html.replace('</body>','<script src="./catalogos-v7.js"></script></body>');
