/* M.A.R.C. — módulo Clientes */
(function(){
  const esc=v=>String(v??'').replace(/[&<>'"]/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;',"'":'&#39;','"':'&quot;'}[c]));
  const oldRender=window.render;
  window.render=function(section='dashboard'){
    if(section!=='clients') return oldRender(section);
    const content=document.querySelector('#content');
    if(!content)return;
    content.innerHTML=`<div class="page-head"><div><p class="eyebrow">M.A.R.C. / OPERACIONES</p><h1>Clientes</h1><p>Empresas y personas, listas para cotizaciones e informes.</p></div></div><section class="panel table-panel"><div class="table-toolbar"><div class="search-box local"><span>⌕</span><input id="clientsSearch" placeholder="Buscar cliente..."></div><button class="btn btn-primary" id="newClientBtn">＋ Nuevo cliente</button></div><div id="clientsTableWrap"><div class="clients-loading">Cargando clientes...</div></div></section>`;
    loadClients();
  };
  async function loadClients(){
    const wrap=document.querySelector('#clientsTableWrap');
    if(!wrap)return;
    const {data:{user}}=await supabaseClient.auth.getUser();
    if(!user){wrap.innerHTML='<div class="clients-empty">Inicia sesión para ver tus clientes.</div>';return;}
    const {data,error}=await supabaseClient.from('clientes').select('id,tipo_documento,documento,nombre_razon_social,nombre_contacto,telefono,correo,direccion,notas').eq('user_id',user.id).order('nombre_razon_social',{ascending:true});
    if(error){wrap.innerHTML=`<div class="clients-empty">No se pudieron cargar los clientes.<br><small>${esc(error.message)}</small></div>`;return;}
    const rows=data||[];
    const draw=list=>{
      if(!list.length){wrap.innerHTML='<div class="clients-empty"><div class="clients-empty-icon">♙</div><h3>No tienes clientes todavía</h3><p>Agrega tu primer cliente para comenzar.</p><button class="btn btn-primary" id="emptyNewClient">＋ Agregar cliente</button></div>';return;}
      wrap.innerHTML=`<div class="table-scroll"><table><thead><tr><th>Cliente / razón social</th><th>Documento</th><th>Contacto</th><th>Teléfono</th><th>Correo</th><th>Acciones</th></tr></thead><tbody>${list.map(c=>`<tr><td><strong>${esc(c.nombre_razon_social)}</strong></td><td>${esc(c.tipo_documento)} ${esc(c.documento)}</td><td>${esc(c.nombre_contacto||'—')}</td><td>${esc(c.telefono||'—')}</td><td>${esc(c.correo||'—')}</td><td><button class="table-action client-edit" data-id="${c.id}">✎</button> <button class="table-action client-delete" data-id="${c.id}">×</button></td></tr>`).join('')}</tbody></table></div><div class="table-foot">Mostrando ${list.length} de ${rows.length} clientes</div>`;
    };
    draw(rows);
    document.querySelector('#clientsSearch')?.addEventListener('input',e=>{const q=e.target.value.toLowerCase().trim();draw(rows.filter(c=>[c.nombre_razon_social,c.documento,c.nombre_contacto,c.telefono,c.correo].some(v=>String(v||'').toLowerCase().includes(q))));});
    const open=()=>openClientModal();document.querySelector('#newClientBtn')?.addEventListener('click',open);document.querySelector('#emptyNewClient')?.addEventListener('click',open);
    document.querySelectorAll('.client-edit').forEach(b=>b.addEventListener('click',()=>openClientModal(rows.find(c=>c.id===b.dataset.id))));
    document.querySelectorAll('.client-delete').forEach(b=>b.addEventListener('click',()=>deleteClient(b.dataset.id,rows)));
  }
  function openClientModal(c=null){
    document.querySelector('#clientModal')?.remove();
