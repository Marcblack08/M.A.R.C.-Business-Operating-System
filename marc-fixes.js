...
(function(){
  ...
  /* El importador de catálogos pertenece exclusivamente a Inventario.
     catalogos-v4.js lo monta dentro de .main; aquí controlamos su visibilidad
     para que nunca aparezca en Dashboard ni en los demás módulos. */
  ...
  if(main){
    new MutationObserver(()=>requestAnimationFrame(syncCatalogVisibility)).observe(main,{childList:true,subtree:true});
  }
  ...
  if(typeof originalShowApp==='function')window.showApp=function(session){
    ...
    originalRender(validSections.has(current)?current:'dashboard');
    requestAnimationFrame(syncCatalogVisibility);
  };
  ...
  window.addEventListener('hashchange',()=>{
    ...
    if(validSections.has(section)){
      current=section;
      if(typeof window.render==='function')window.render(section);
      else syncCatalogVisibility();
    }
  });
