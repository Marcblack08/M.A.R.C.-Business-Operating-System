...
async function cashMovementModal(open,type){
  ...
  const close=modal('<div class="cash-pos-modal"><div class="modal-head cash-pos-head"><div><div class="eyebrow2">PUNTO DE VENTA</div><h2>🛒 Nueva venta</h2><p>Selecciona productos, cantidades y cobra todo en una sola operación.</p></div><button class="close" id="x">×</button></div><div id="pos" class="cash-pos-layout"><section class="cash-pos-products"><input id="posSearch" class="cash-product-search" placeholder="🔎 Buscar producto, marca, modelo o SKU…" autofocus><div id="posCats" class="cash-pos-cats"></div><div id="posProducts" class="cash-pos-product-grid"></div></section><aside class="cash-pos-cart"><div class="cash-pos-cart-head"><div><b>Tu pedido</b><small>Toca un producto para añadirlo</small></div><span id="posCount" class="eyebrow2">0</span></div><div id="posCart" class="cash-pos-cart-list"></div><div class="cash-pos-cart-total"><div><b>Total</b><strong id="posTotal">S/ 0.00</strong></div><label>Cliente / referencia opcional<input id="posReference" placeholder="Nombre, boleta, nota…"></label><button type="button" class="primary cash-pos-charge" id="posCharge">Cobrar venta · S/ 0.00</button></div></aside></div></div><div style="display:flex;justify-content:space-between;align-items:center;gap:8px"><div><b>Tu pedido</b><small style="display:block;opacity:.65">Toca un producto para añadirlo</small></div><span id="posCount" class="eyebrow2">0</span></div><div id="posCart" style="overflow:auto;flex:1;margin:10px 0"></div><div style="border-top:1px solid var(--line,#ddd);padding-top:12px"><div style="display:flex;justify-content:space-between;font-size:1.15rem"><b>Total</b><strong id="posTotal">S/ 0.00</strong></div><label style="margin-top:10px">Cliente / referencia opcional<input id="posReference" placeholder="Nombre, boleta, nota…"></label><button type="button" class="primary" id="posCharge" style="width:100%;margin-top:10px;padding:14px">Cobrar venta · S/ 0.00</button></div></aside></div>');
  $("#x").onclick=close;
  ...
  const addProduct=p=>{if(!p||Number(p.stock||0)<=0)return;const item=cart.get(p.id);if(item){if(item.quantity>=Number(p.stock||0))return toast("Ya no queda más stock de "+p.name,"err");item.quantity++}else cart.set(p.id,{product:p,quantity:1});renderCart();renderProducts()};
  const changeQty=(id,delta)=>{const item=cart.get(id);if(!item)return;const next=item.quantity+delta;if(next<=0)cart.delete(id);else if(next<=Number(item.product.stock||0))item.quantity=next;else return toast("Ese stock ya se acabó 😅","err");renderCart();renderProducts()};
