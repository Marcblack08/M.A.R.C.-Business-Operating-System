...
async function enhance(){
 ...
 try{
  ...
  const [pr,ci]=await Promise.all([
   supabaseClient.from('productos').select('id,codigo,nombre,descripcion,precio_compra,precio_venta,proveedor').eq('user_id',u.id).eq('activo',true),
  ...
  const purchaseTh=document.createElement('th');purchaseTh.textContent='Precio compra';
  const saleTh=document.createElement('th');saleTh.textContent='Precio venta';
  const providerTh=document.createElement('th');providerTh.textContent='Proveedor';
  const originTh=document.createElement('th');originTh.textContent='Origen';
  head.insertBefore(purchaseTh,action);head.insertBefore(saleTh,action);head.insertBefore(providerTh,action);head.insertBefore(originTh,action);
  table.querySelectorAll('tbody tr').forEach(tr=>{
   ...
   const purchase=document.createElement('td');purchase.innerHTML=`<div class="inv-data-edit"><span>S/</span><input class="inv-purchase-price" data-id="${esc(p.id)}" type="number" min="0" step="0.01" value="${Number(p.precio_compra||0).toFixed(2)}"><button type="button" class="inv-save-purchase" data-id="${esc(p.id)}" title="Guardar precio de compra">✓</button></div>`;
   const sale=document.createElement('td');sale.innerHTML=`<div class="inv-data-edit"><span>S/</span><input class="inv-sale-price" data-id="${esc(p.id)}" type="number" min="0" step="0.01" value="${Number(p.precio_venta||0).toFixed(2)}"><button type="button" class="inv-save-sale" data-id="${esc(p.id)}" title="Guardar precio de venta">✓</button></div>`;
   const provider=document.createElement('td');provider.innerHTML=`<div class="inv-provider-edit"><input class="inv-provider" data-id="${esc(p.id)}" type="text" maxlength="160" placeholder="Nombre proveedor" value="${esc(p.proveedor||'')}"><button type="button" class="inv-save-provider" data-id="${esc(p.id)}" title="Guardar proveedor">✓</button></div>`;
   const origin=document.createElement('td');origin.innerHTML=badge(supplier);
   const actionCell=actionBtn.closest('td');tr.insertBefore(purchase,actionCell);tr.insertBefore(sale,actionCell);tr.insertBefore(provider,actionCell);tr.insertBefore(origin,actionCell);tr.classList.toggle('inv-supplier-row',supplier);
  });
  ...
  wrap.querySelectorAll('.inv-save-purchase').forEach(btn=>btn.onclick=async()=>{const input=wrap.querySelector(`.inv-purchase-price[data-id="${CSS.escape(btn.dataset.id)}"]`),value=Number(input?.value);if(!Number.isFinite(value)||value<0){alert('Ingresa un precio de compra válido.');return}btn.disabled=true;try{const r=await supabaseClient.from('productos').update({precio_compra:value,updated_at:new Date().toISOString()}).eq('id',btn.dataset.id).eq('user_id',u.id);if(r.error)throw r.error;btn.textContent='✓'}catch(e){alert('No se pudo guardar el precio de compra: '+e.message)}finally{btn.disabled=false}});
  wrap.querySelectorAll('.inv-save-sale').forEach(btn=>btn.onclick=async()=>{const input=wrap.querySelector(`.inv-sale-price[data-id="${CSS.escape(btn.dataset.id)}"]`),value=Number(input?.value);if(!Number.isFinite(value)||value<0){alert('Ingresa un precio de venta válido.');return}btn.disabled=true;try{const r=await supabaseClient.from('productos').update({precio_venta:value,updated_at:new Date().toISOString()}).eq('id',btn.dataset.id).eq('user_id',u.id);if(r.error)throw r.error;btn.textContent='✓'}catch(e){alert('No se pudo guardar el precio de venta: '+e.message)}finally{btn.disabled=false}});
  wrap.querySelectorAll('.inv-save-provider').forEach(btn=>btn.onclick=async()=>{const input=wrap.querySelector(`.inv-provider[data-id="${CSS.escape(btn.dataset.id)}"]`),value=String(input?.value||'').trim();btn.disabled=true;try{const r=await supabaseClient.from('productos').update({proveedor:value||null,updated_at:new Date().toISOString()}).eq('id',btn.dataset.id).eq('user_id',u.id);if(r.error)throw r.error;btn.textContent='✓'}catch(e){alert('No se pudo guardar el proveedor: '+e.message)}finally{btn.disabled=false}});
