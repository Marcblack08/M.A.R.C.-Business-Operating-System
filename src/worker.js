...
const CATALOG_SCHEMA={type:'object',properties:{items:{type:'array',items:{type:'object',properties:{codigo:{type:'string'},nombre:{type:'string'},marca:{type:'string'},modelo:{type:'string'},categoria:{type:'string'},descripcion:{type:'string'},precio_compra:{type:'number'},precio_venta:{type:'number'},precio_fuente:{type:'number'},unidad:{type:'string'},unidad_precio:{type:'string'},cantidad_paquete:{type:'number'},presentacion:{type:'string'},confianza:{type:'number'}},required:['codigo','nombre','marca','modelo','categoria','descripcion','precio_compra','precio_venta','precio_fuente','unidad','unidad_precio','cantidad_paquete','presentacion','confianza']}}},required:['items']};
...
12. Devuelve ÚNICAMENTE JSON con {"items":[...]}.`;
...
async function analyzeCatalogText(env,text,tipo='PROVEEDOR'){
  ...
  try{const r=await env.AI.run('@cf/meta/llama-3.1-8b-instruct-fast',{messages:[{role:'system',content:'Eres M.A.R.C. Devuelve exclusivamente el JSON solicitado.'},{role:'user',content:prompt}],max_tokens:2200,temperature:0,response_format:{type:'json_schema',json_schema:CATALOG_SCHEMA}});const d=normalizeData(r?.response||r);if(Array.isArray(d.items))return d.items;first=new Error('Respuesta sin items');}catch(e){first=e;}
  try{const r=await env.AI.run('@cf/meta/llama-3.1-8b-instruct-fast',{messages:[{role:'system',content:'Devuelve SOLO JSON válido.'},{role:'user',content:`${prompt}\nFormato exacto: {"items":[]}`}],max_tokens:2200,temperature:0});const d=normalizeData(r?.response||r);if(Array.isArray(d.items))return d.items;throw first||new Error('Sin items');}catch(e){throw new Error(`No se pudo analizar el texto del catálogo: ${String(e?.message||e)}`);}
}
