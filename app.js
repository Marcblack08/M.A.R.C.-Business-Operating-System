    ...
    <div class="dashboard-shell">
      <section class="dashboard-hero">
        ...
        <div class="hero-visual" aria-hidden="true">
          ...
          <div class="hero-visual-logo is-marc-brand" id="heroBrandLogo" aria-label="M.A.R.C."><span>M</span></div>
...
async function agenda(){
 ...
 $("#appointmentForm").onsubmit=async e=>{e.preventDefault();const b=e.submitter;b.disabled=true;try{const d=new FormData(e.currentTarget);const payload={user_id:st.u.id,client_id:String(d.get("client_id")||"")||null,title:String(d.get("title")||"").trim(),description:String(d.get("description")||"").trim()||null,start_at:new Date(String(d.get("start_at"))).toISOString(),end_at:d.get("end_at")?new Date(String(d.get("end_at"))).toISOString():null,status:String(d.get("status")),technician:String(d.get("technician")||"").trim()||null,location:String(d.get("location")||"").trim()||null,reminder_minutes:Number(d.get("reminder_minutes")||60),updated_at:new Date().toISOString()};const r=isEdit?await S.from("marc_appointments").update(payload).eq("id",existing.id).eq("user_id",st.u.id):await S.from("marc_appointments").insert(payload);if(r.error)throw r.error;close();toast(isEdit?"Cita actualizada":"Cita creada","ok");agenda()}catch(err){toast(err.message||"No se pudo guardar la cita","err");b.disabled=false}};
}
function maintenancePlanModal(existing=null){
 const isEdit=Boolean(existing?.id);
 const close=modal('<div class="modal-head"><div><div class="eyebrow2">MANTENIMIENTO RECURRENTE</div><h2>'+(isEdit?"Editar plan":"Nuevo plan")+'</h2><p>Programa automáticamente la siguiente visita después de cada mantenimiento.</p></div><button class="close" id="maintenanceClose">×</button></div><form id="maintenanceForm" class="form-grid"><label>Cliente<select name="client_id"><option value="">Sin cliente</option></select></label><label>Nombre del plan<input name="title" required value="'+esc(existing?.title||"Mantenimiento preventivo")+'"></label><label>Tipo de servicio<input name="service_type" value="'+esc(existing?.service_type||"")+'" placeholder="CCTV, red, PC, NVR…"></label><label>Intervalo<select name="interval_months"><option value="1">Mensual</option><option value="3">Cada 3 meses</option><option value="6">Cada 6 meses</option><option value="12">Anual</option><option value="24">Cada 2 años</option></select></label><label>Próximo mantenimiento<input name="next_due_at" type="datetime-local" required></label><label>Técnico<input name="technician" value="'+esc(existing?.technician||"")+'"></label><label>Ubicación<input name="location" value="'+esc(existing?.location||"")+'"></label><label style="grid-column:1/-1">Notas<textarea name="notes" rows="3">'+esc(existing?.notes||"")+'</textarea></label><div class="modal-actions" style="grid-column:1/-1"><button type="button" class="secondary" id="maintenanceCancel">Cancelar</button><button class="primary">Guardar plan</button></div></form>');
...
async function serviceOrders(){
  ...
  '<section class="service-profitability-summary"><div><span>INGRESOS COBRADOS</span><strong>'+money(revenue)+'</strong><small>Importe registrado en las OT</small></div><div><span>COSTO TOTAL REAL</span><strong>'+money(costs)+'</strong><small>Mano de obra + transporte + materiales + otros</small></div><div><span>UTILIDAD ACUMULADA</span><strong class="'+(profit>=0?"ok":"out")+'">'+money(profit)+'</strong><small>Ingresos menos todos los costos</small></div><div><span>MARGEN GLOBAL</span><strong>'+(margin>=0?"":"-")+formatPct(Math.abs(margin))+'</strong><small>'+rows.length+' órdenes analizadas</small></div></section>'+
  ...
  const draw=(items)=>{
    $("#serviceOrderRows").innerHTML=items.map(r=>{
      const rev=Number(r.revenue??0),cost=Number(r.labor_cost??0)+Number(r.transport_cost??0)+Number(r.materials_cost??0)+Number(r.other_cost??0),p=Number(r.profit??(rev-cost)),m=rev>0?(p/rev)*100:0;
    ...
    const visible=items;
    $("#serviceMobileCards").innerHTML=visible.map(r=>mobileCardsFor(r)).join("")||'<div class="empty">No hay órdenes de trabajo todavía.</div>';
  };
  const mobileCardsFor=r=>{
    const rev=Number(r.revenue??0),cost=Number(r.labor_cost??0)+Number(r.transport_cost??0)+Number(r.materials_cost??0)+Number(r.other_cost??0),p=Number(r.profit??(rev-cost)),m=rev>0?(p/rev)*100:0;
    return '<article class="service-mobile-card"><div class="service-mobile-top"><div class="service-mobile-title"><b>'+esc(r.number||"OT")+' · '+esc(r.title||"Servicio")+'</b><small>'+esc(r.service_type||"Servicio")+'</small></div><span class="service-mobile-status">'+esc(r.status||"PENDIENTE")+'</span></div><div class="service-mobile-client">👤 '+esc(r.marc_clients?.name||"Sin cliente")+(r.location?" · 📍 "+esc(r.location):"")+'</div><div class="service-mobile-grid"><div class="service-mobile-metric"><span>Cobrado</span><b>'+money(rev)+'</b></div><div class="service-mobile-metric"><span>Costo real</span><b>'+money(cost)+'</b></div><div class="service-mobile-metric service-mobile-profit"><span>Utilidad</span><b>'+money(p)+'</b></div><div class="service-mobile-metric"><span>Margen</span><b>'+formatPct(m)+'</b></div></div><small>'+(r.scheduled_at?"📅 "+esc(new Date(r.scheduled_at).toLocaleString("es-PE",{dateStyle:"medium",timeStyle:"short"})):"Sin programación")+'</small>'+cardActions(r)+'</article>';
  };
  draw(rows);
