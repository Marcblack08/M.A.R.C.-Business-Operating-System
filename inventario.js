  ...
  async function loadInventory(){
    ...
    try{
      ...
      function draw(list){
        ...
        wrap.innerHTML=`<div class="table-scroll"><table><thead><tr><th>Código</th><th>Producto</th><th>Stock</th><th>Mínimo</th><th>Estado</th><th>Acción</th></tr></thead><tbody>${list.map(p=>{const stock=Number(p.stock||0),min=Number(p.stock_minimo||0),status=stock<=0?'Agotado':stock<=min?'Bajo':'Disponible';return `<tr><td><strong>${esc(p.codigo||'—')}</strong></td><td><strong>${esc(p.nombre)}</strong><br><small>${esc([p.marca,p.modelo].filter(Boolean).join(' · '))}</small></td><td><strong>${stock}</strong> ${esc(p.unidad||'UND')}</td><td>${min}</td><td><mark class="${status==='Agotado'?'critical':status==='Bajo'?'warning':''}">${status}</mark></td><td><div style="display:flex;gap:6px;flex-wrap:wrap"><button type="button" class="table-action inv-move" data-id="${p.id}" title="Registrar movimiento">＋/−</button><button type="button" class="table-action inv-delete" data-id="${p.id}" title="Eliminar del inventario" aria-label="Eliminar ${esc(p.nombre)}">🗑</button></div></td></tr>`;}).join('')}</tbody></table></div>`;
        wrap.querySelectorAll('.inv-move').forEach(b=>b.addEventListener('click',()=>openMovementModal(rows.find(p=>p.id===b.dataset.id))));
        wrap.querySelectorAll('.inv-delete').forEach(b=>b.addEventListener('click',()=>removeFromInventory(rows.find(p=>p.id===b.dataset.id))));
      }
    }catch(err){wrap.innerHTML=`<div class="inventory-empty">No se pudo cargar el inventario.<br><small>${esc(err.message)}</small></div>`;}
  }
  async function removeFromInventory(product){
    if(!product)return;
    const name=product.nombre||product.codigo||'este producto';
    const confirmed=window.confirm(`¿Eliminar "${name}" del inventario?\n\nEl producto se retirará del inventario, pero permanecerá en Productos y no se borrará su historial.`);
    if(!confirmed)return;
    try{
      const user=await getUser();
      const {error}=await supabaseClient.from('productos').update({activo:false}).eq('id',product.id).eq('user_id',user.id);
      if(error)throw error;
      await loadInventory();
    }catch(err){window.alert('No se pudo eliminar del inventario: '+(err.message||err));}
  }
