...
function boot(){improveButtonReliability();let queued=false;let timer=0;const scheduleEnhance=()=>{if(queued)return;queued=true;clearTimeout(timer);timer=setTimeout(()=>{queued=false;enhance()},70)};const observer=new MutationObserver(scheduleEnhance);[$("#content"),$("#modal")].filter(Boolean).forEach(root=>observer.observe(root,{childList:true,subtree:true}));later(enhance,120);window.addEventListener("resize",()=>{if(window.innerWidth>760)toggleMore(false)})}
