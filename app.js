...
async function marketing(){
  ...
  $("#content").innerHTML=`
    ...
    <div class="marketing-benefits"><div><b>✦ Imagen con IA</b><small>2 propuestas visuales para elegir</small></div><div><b>✓ Texto automático</b><small>Descripción, CTA y hashtags</small></div><div><b>◉ Datos de tu empresa</b><small>Logo, nombre y teléfono</small></div><div><b>📱 Compartir desde tu teléfono</b><small>WhatsApp, Instagram y más</small></div></div>
  ...
  const renderedBannerData=()=>{try{const canvas=$p("adCanvas");return canvas&&canvas.width&&canvas.height?canvas.toDataURL("image/png"):""}catch{return""}};
  ...
  $p("shareAd").onclick=()=>sharePublication("SHARE");
...
function wire(){
  ...
  const bindThemeButton=(selector)=>{const el=$(selector);if(!el)return;el.type="button";el.onclick=toggleTheme;};
