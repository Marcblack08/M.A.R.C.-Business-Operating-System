...
async function marketingAi(request,env){
  ...
  const prompt={messages:[
    {role:"system",content:'Eres el director creativo y copywriter de M.A.R.C. El usuario puede darte la información de forma libre y conversacional. Primero interpreta el brief y completa internamente lo que falte: objetivo, público, tono y enfoque comercial. Si un dato no está indicado, elige una formulación prudente y genérica, pero NUNCA inventes características técnicas, certificaciones, garantías, descuentos, disponibilidad, medidas, marcas, precios, resultados ni beneficios no sustentados. El precio del inventario solo puede usarse como precio cuando sea pertinente y no contradiga una oferta indicada por el usuario. Prioriza claridad, venta y una llamada a la acción natural. Devuelve SOLO JSON válido con title, headline, primary_text, short_text, whatsapp_text, banner_text, hashtags. headline breve para banner; primary_text completo para redes; short_text corto; whatsapp_text natural y termina con el CTA; banner_text máximo 3 líneas; hashtags arreglo de 3 a 8.'},
    {role:"user",content:"PLATAFORMA: "+platform+"\nOBJETIVO (puede venir vacío): "+String(body?.objective||"")+"\nTONO (puede venir vacío): "+String(body?.tone||"")+"\nPÚBLICO (puede venir vacío): "+String(body?.audience||"")+"\nOFERTA (puede venir vacía): "+String(body?.offer||"")+"\nCTA (puede venir vacío): "+String(body?.cta||"")+"\nBRIEF DEL USUARIO: "+String(body?.details||"")+"\nPRODUCTO: "+JSON.stringify({name:String(p.name||"Producto"),brand:String(p.brand||""),model:String(p.model||""),category:String(p.category||""),price:p.price==null||p.price===""?null:Number(p.price),stock:p.stock==null||p.stock===""?null:Number(p.stock)})}
  ]};
