...
export default {async fetch(request,env,ctx){
 ...
 if(request.method==='POST'&&url.pathname==='/api/analyze-catalog-page'){
  try{
   ...
   try{return Response.json({items:await gemini(env,img,tipo,pagina),model:MODEL,pagina})}catch(g){
    try{return Response.json({items:await cloudflare(env,img,tipo,pagina),model:CF_MODEL,pagina,backup:true})}catch(c){
     const gm=String(g?.message||g),cm=String(c?.message||c);const quota=/quota|429|daily free allocation|10,000 neurons|3036|4006/i.test(cm)||g?.quota;
     return Response.json({error:quota?'Límite de IA alcanzado.':'No se pudo analizar la página.',detail:quota?'Gemini no respondió y Cloudflare Workers AI tampoco está disponible por cuota.':`${gm} | Respaldo: ${cm}`,quota},{status:quota?429:503});
    }
   }
