...
(function(){
  ...
  async function sync(){
    ...
    try{
      ...
      const rows=data||[],own=rows.filter(p=>!isSupplier(p));
      ...
      wrap.querySelectorAll('tbody tr').forEach(tr=>{
        const btn=tr.querySelector('.inv-move'),p=btn?rows.find(x=>String(x.id)===String(btn.dataset.id)):null;
        if(!p)return;
        const supplier=isSupplier(p),stock=Number(p.stock||0),min=Number(p.stock_minimo||0);
        const status=supplier&&stock<=0?'Catálogo proveedor':stock<=0?'Agotado':stock<=min?'Bajo':'Disponible';
        ...
        tr.style.display=(supplier&&stock<=0&&active!=='all')?'none':'';
      });
  ...
  function start(){
    ...
    const obs=new MutationObserver(mutations=>{
      const hasElementAdded=mutations.some(m=>[...m.addedNodes].some(n=>n.nodeType===1));
      if(hasElementAdded&&document.querySelector('#inventoryTableWrap'))setTimeout(sync,80);
    });
    obs.observe(document.body,{childList:true,subtree:true});
    setInterval(()=>{if(document.querySelector('#inventoryTableWrap'))sync()},5000);
  }
