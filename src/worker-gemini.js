export default {
  async fetch(request, env) {
    ...
    try {
      ...
      const normalize=j=>Array.isArray(j?.items)?j.items.map(x=>({codigo:String(x?.codigo||'').trim(),nombre:String(x?.nombre||'').trim(),marca:String(x?.marca||'').trim(),modelo:String(x?.modelo||'').trim(),categoria:String(x?.categoria||'').trim(),descripcion:String(x?.descripcion||'').trim(),precio_fuente:Number(x?.precio_fuente||0),unidad:String(x?.unidad||'UND').trim(),unidad_precio:String(x?.unidad_precio||'').trim(),cantidad_paquete:Number(x?.cantidad_paquete||1),presentacion:String(x?.presentacion||'').trim(),variante:String(x?.variante||'').trim()})):[];
      const providers=[];
      if(env.GEMINI_API_KEY) providers.push(async()=>{const r=await fetch('https://generativelanguage.googleapis.com/v1beta/models/gemini-3.6-flash:generateContent?key='+encodeURIComponent(env.GEMINI_API_KEY),{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({contents:[{role:'user',parts:[{text:prompt},{inline_data:{mime_type:'image/jpeg',data:image.split(',').pop()}}]}],generationConfig:{responseMimeType:'application/json',responseSchema:schema,maxOutputTokens:1800}})});const j=await r.json();if(!r.ok)throw new Error(j?.error?.message||'Gemini no disponible');return normalize(JSON.parse(j?.candidates?.[0]?.content?.parts?.map(p=>p.text||'').join('')||'{}'));});
      if(env.OPENROUTER_API_KEY) providers.push(async()=>{const r=await fetch('https://openrouter.ai/api/v1/chat/completions',{method:'POST',headers:{'Content-Type':'application/json',Authorization:'Bearer '+env.OPENROUTER_API_KEY},body:JSON.stringify({model:'openrouter/free',messages:[{role:'user',content:[{type:'text',text:prompt},{type:'image_url',image_url:{url:image}}]}],response_format:{type:'json_object'},max_tokens:1800})});const j=await r.json();if(!r.ok)throw new Error(j?.error?.message||'OpenRouter no disponible');return normalize(JSON.parse(j?.choices?.[0]?.message?.content||'{}'));});
      if(env.AI) providers.push(async()=>{const r=await env.AI.run('@cf/google/gemma-4-26b-a4b-it',{prompt,image:image.split(',').pop()});return normalize(typeof r==='string'?JSON.parse(r):r);});
      let lastError=null;
      for(const provider of providers){try{const items=await provider();if(items.length)return Response.json({items,pagina,tipo,fuente:'IA visual + OCR'});}catch(e){lastError=e;}}
