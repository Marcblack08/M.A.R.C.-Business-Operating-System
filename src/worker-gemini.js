   ...
   try{return Response.json({items:await openrouter(env,img,tipo,pagina),model:OPENROUTER_MODEL,pagina,provider:'openrouter'})}catch(o){
    try{return Response.json({items:await gemini(env,img,tipo,pagina),model:MODEL,pagina,provider:'gemini'})}catch(g){
     try{return Response.json({items:await cloudflare(env,img,tipo,pagina),model:CF_MODEL,pagina,backup:true,provider:'cloudflare'})}catch(c){
      const om=String(o?.message||o),gm=String(g?.message||g),cm=String(c?.message||c);const quota=/quota|429|rate limit|daily free allocation|10,000 neurons|3036|4006/i.test(om)||/quota|429/i.test(gm)||/quota|429|daily free allocation|10,000 neurons|3036|4006/i.test(cm)||o?.quota||g?.quota;
      return Response.json({error:quota?'Límite de IA alcanzado.':'No se pudo analizar la página.',detail:`OpenRouter: ${om} | Gemini: ${gm} | Cloudflare: ${cm}`,quota},{status:quota?429:503});
     }
    }
   }
